...
class HeadlessGodotExecutor {
    constructor(config) {
        this.validatedPaths = new Map();
        this.strictPathValidation = false;
    ...
    logDebug(message) {
        if (DEBUG_MODE)
            console.debug(`[HEADLESS DEBUG] ${message}`);
    }
    async detectGodotPath() {
        if (godotPath && await this.isValidGodotPath(godotPath))
            return;
    ...
    async runProject(projectPath, scene) {
        if (activeProcess)
...
const allTools = [...nodeTools, ...scriptTools, ...sceneTools, ...editorTools, {
        ...
        execute: async (args) => new HeadlessGodotExecutor().launchEditor(args.projectPath),
    }, {
        name: 'run_project',
        ...
        execute: async (args) => new HeadlessGodotExecutor().runProject(args.projectPath, args.scene),
    }, {
